...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    ...
    // children_.insert(std::make_pair(key_char, move(child)));
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    auto node = &root_;
    if (key.size() > 1) {
      for (char key_char : key.substr(0, key.size() - 1)) {
        if (!node->get()->HasChild(key_char)) {
          node->get()->InsertChildNode(key_char, std::make_unique<TrieNode>(key_char));
        }
      ...
      if (node->get()->HasChildren()) {
        ...
        for (; i > -1; --i) {
          if (node->get()->IsEndNode() || node->get()->HasChildren()) {
            nodes[i]->get()->RemoveChildNode(key[i]);
            node = nodes[i];
          } else {
            break;
          }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto tmp = dynamic_cast<TrieNodeWithValue<T> *>(node->get());
