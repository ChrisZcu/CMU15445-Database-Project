...
namespace bustub
{
  ...
  public:
    ...
    auto DFS(txn_id_t txn_id) -> bool
    {
      ...
      for (auto const next_node : next_node_vector)
      {
  ...
  private:
    ...
    std::unordered_map<table_oid_t, std::shared_ptr<LockRequestQueue>> table_lock_map_; // 记录表的锁请求
    /** Coordination */
    std::mutex table_lock_map_latch_;

    /** Structure that holds lock requests for a given RID */
    std::unordered_map<RID, std::shared_ptr<LockRequestQueue>> row_lock_map_; // 行和锁请求
    /** Coordination */
